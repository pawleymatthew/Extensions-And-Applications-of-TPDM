...
\begin{equation}\label{eq-empirical-local-tpdm-normality}
    \sqrt{k}(\hat{\bm{\sigma}}(t) - \bm{\sigma}(t)) \to N(\bm{0}, V)
\end{equation}
as $n\to\infty$. The $\mathcal{D}\times\mathcal{D}$ asymptotic covariance matrix $V=V(t)$ has entries given by
\begin{equation*}
   v_{ij,lm}(t) 
...
The drift and diffusion coefficients associated with each univariate limiting process $\{Y_{ij}(t):t\in[0,1]\}$ are controlled by $\{\psi_{ij}(t):t\in[0,1]\}$ and $\{\nu_{ij}^2(t):t\in[0,1]\}$, respectively. Meanwhile, the cross-correlation between $\{Y_{ij}(t):t\in[0,1]\}$ and $\{Y_{lm}(t):t\in[0,1]\}$ is determined by $\{\rho_{ij,lm}(t):t\in[0,1]\}$. Ordinarily, functional central limit theorem results scale with $\sqrt{n}$, whereas here $\sqrt{k/2h}$ is the correct scaling factor, since it represents the square root of the effective sample size (total number of extreme observations across all blocks).
...
\hat{\bm{Z}}(t) := \sqrt{\frac{k}{2h}} V^{-1/2}(\hat{\bm{\psi}}(t) - t\hat{\bm{\psi}}(1)).
\end{equation}
Then $\{\hat{\bm{Z}}(t) : t\in[0,1]\}$ converges to $\{\bm{B}(t):t\in[0,1]\}$, a standard $\mathcal{D}$-dimensional Brownian bridge.
:::

Note that $V$ is a matrix, so $V^{-1/2}$ and the multiplication in \eqref{eq-test-process} should be interpreted accordingly. Implicitly, we assume that the covariance matrix is invertible and positive definite; this is discussed in more depth when we detail our inference procedure.
...
Next, we return to investigations about the power of the test and check whether it detects dependence changes with high probability when the number of samples is large. The experimental procedure is unchanged from before, except that the sample size is $n=10^6$ and data are generated from the SL-jump model with $\vartheta_0=2$ and $\vartheta_1=2.5$. These parameters bring about relatively subtle shifts in the dependence strength, with $\sigma_{ij}(t)=0.85$ for $t<0.5$ and $\sigma_{ij}(t)=0.91$ for $t\geq 0.5$. However, the sheer abundance of data ensures that even these very subtle changes are detected. Across 300 repeated simulations, all the tests' p-values are less than $10^{-15}$ meaning our method consistently and overwhelmingly identifies the dependence change. (A graphical/tabular representation of the empirical distribution of the p-values is omitted because it simply shows lots of values concentrated near zero.) To fully explore the asymptotic properties of the proposed methods, in the next sections we investigate how they perform for more moderate sample sizes.
...
From the original gridded data set, we select 200 sites at random before dividing the spatial domain into northerly and southerly sub-regions, each comprising 70 sites as shown in @fig-redsea-sites. @simpsonConditionalModellingSpatiotemporal2021 advise treating these areas separately because surface temperature extremes exhibit differing behaviour in the north and south. Additionally, they show that, at any particular location, high temperatures may persist across several days resulting in (temporal) clusters of extremes of daily maxima. To reduce serial dependence we instead work with weekly maxima, yielding $n=1605$ samples spanning approximately 31 years. An extremogram is used to check that serial dependence is reduced to an acceptable level. Let $X_i^{(\text{north})}(t)$ and $X_i^{(\text{south})}(t)$ denote the surface temperature anomaly (on stationary 2-Fréchet margins) at site $i\in\{1,\ldots,70\}$ and time $t\in[0,1]$ in the two sub-regions. Our goal is to determine whether it is reasonable to assume stationary dependence for either/both of
\begin{align*}
