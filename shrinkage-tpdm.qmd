...
## Regularised TPDM estimators: thresholding and shrinkage
...
#| fig-cap: "Illustration of popular thresholding operators $s_{\\lambda}(x)$ with $\\lambda=0.1$."
#| fig-scap: "Illustration of popular thresholding operators $s_{\\lambda}(x)$ with $\\lambda=0.1$."
#| fig-height: 3.5
...
#| fig-scap: "Eigenvalues of the hard-thresholded TPDMs corresponding to the Red Sea surface temperature data."
#| fig-height: 3.5
...
\end{equation*}
The identity matrix is trivially completely positive since $I_d=I_dI_d^T$ and $\hat{\Sigma}$ is completely positive by @prp-empirical-tpdm-completely-positive. The class of completely positive matrices is a convex cone [@bermanCompletelyPositiveMatrices2003], so complete positivity is preserved under convex combinations. 
:::
...
```{r make-fig-frob-risk-example}
#| label: fig-frob-risk-example
#| fig-cap: "Blah."
#| fig-scap: "Blah."
#| fig-height: 3.5
#| eval: false

# @fig-frob-risk-example depicts the Frobenius risk function $\mathcal{R}(\lambda)$ in the case where $\bm{X}$ is five-dimensional Hüsler-Reiss random vector with a fixed parameter matrix and the empirical TPDM is estimated from $k=500$ extremes out of $n=5,000$ independent realisations of $\bm{X}$. The true TPDM is computed using @exm-husler-reiss-tpdm. The best estimator is the adaptive lasso, for which the associated oracle and risk are $\lambda^\star\approx 0.12$ and $\mathcal{R}(\lambda^\star)= 4.2\times 10^{-4}$. The Ledoit-Wolf is worst performing with $\mathcal{R}(\lambda^\star)=0.015$ attained at $\lambda^\star=0.70$. 

res <- readRDS("scripts/shrinkage-tpdm/results/frob-risk-example.RDS")
ggplot(res, aes(x = lambda, y = risk, colour = method)) +
  geom_line() +
  geom_point(aes(x = lambda_star, y = min_risk, colour = method)) +
  scale_x_continuous(limits = c(0, 1), expand = c(0, 0), breaks = breaks_extended(n = 5)) +
  scale_y_continuous(limits = c(0, NA), expand = expansion(mult = c(0, 0.02)), breaks = breaks_extended(n = 5)) +
  scale_colour_manual(values = c("red", "blue", "black", "darkgreen"), labels = c("Adaptive lasso", "Hard", "Ledoit-Wolf", "Soft")) +
  labs(x = expression(lambda), y = "Frobenius risk", colour = "Method") +
  theme_light()
```
...
\begin{align*}
    ...
    &= \lim_{n\to\infty} \sum_{i<j}k(n)\left\lbrace \mathbb{E}[\hat{\sigma}_{ij}(n)^2] - \mathbb{E}[\hat{\sigma}_{ij}(n)]^2\right\rbrace \\
    &= \lim_{n\to\infty} \sum_{i<j}k(n)\mathrm{Var}(\hat{\sigma}_{ij}(n) \\
    &= \sum_{i<j}\nu_{ij}^2.
\end{align*}
**What's the best way of doing Line 2 to line 3? Am just using the fact that $\mathbb{E}[\hat{\sigma}_{ij}]\to\sigma_{ij}$.** Since $\sum_{i<j}\nu_{ij}^2>0$ and $\|\hat{\bm{\sigma}}(n)\|_2^2>0$ almost surely, it follows that $\lambda^\star(n)>0$ for sufficiently large $n$. The above result gives that, asymptotically, the optimal shrinkage behaves as a constant divided by $k(n)\mathbb{E}[\|\hat{\bm{\sigma}}(n)\|_2^2]$. As the asymptotic constant is unknown, we might naively try estimating the asymptotic variances $\nu_{ij}^2$. Indeed, this is the approach taken in the context of covariance matrices and a consistent estimator $\hat{\nu}_{ij}^2$ of $\nu_{ij}^2$ is given in \eqref{eq-asymptotic-variance-estimator} and used in Chapter XX. However, for a finite sample size $\hat{\nu}_{ij}^2$ is biased in a way that mirrors the bias in the empirical TPDM, so this approach is not viable. Instead, we are forced to circumvent estimation of $\sum_{i<j}\nu_{ij}^2$. To eliminate the asymptotic constant, we take ratios at different values of $n$. Given a fixed proportion $\kappa \in (0,1)$, it is easy to see that
\begin{equation*}
...
#| fig-scap: "Optimal/estimated Ledoit-Wolf shrinkage intensities for symmetric logistic data."
#| fig-height: 4.5
#| warning: false
...
#| fig-scap: "The empirical TPDM $\\hat{\\Sigma}$ (left) and the Ledoit-Wolf TPDM $\\tilde{\\Sigma}$ for the EVA (2023) Data Challenge."
#| fig-height: 4
...
#| fig-scap: "Estimated shrinkage intensities $\\hat{\\lambda}^{(1)},\\ldots,\\hat{\\lambda}^{(5)}$ for the EVA (2023) Data Challenge."
#| fig-height: 2.5
#| fig-width: 5
#| message: false
...
#| fig-scap: "Probability estimates for the EVA (2023) Data Challenge obtained from the Ledoit-Wolf TPDM."
#| fig-height: 4.5
#| message: false
...
Our final experiment emulates the simulation study in @fixSimultaneousAutoregressiveModels2021. The goal is to estimate the dependence parameter $\rho$ of the extremal SAR model (Section XX), a sub-case of the max-linear model used for spatial extremes. This experiment will reinforce the effectiveness of our tuning procedure, while also highlighting the limitations of Ledoit-Wolf shrinkage compared to the more flexible thresholding estimators. Suppose $\bm{X}$ is as described in Section XX, with (known) neighbourhood matrix $W$ and (unknown) dependence parameter $\rho$. Following @fixSimultaneousAutoregressiveModels2021, $\rho$ is to be estimated by minimising the Frobenius difference between the model TPDM (viewed as a function of $\rho$) and a bias-corrected estimate of the TPDM. This can be posed as a hierarchical/bilevel optimisation problem, whereby the upper-level objective function (estimating $\rho$) depends on the solution to a lower-level problem (estimating $\Sigma$). Specifically, we define
...
#| fig-scap: "Results on regularised TPDM estimation for fitting extremal SAR models."
#| fig-height: 4
