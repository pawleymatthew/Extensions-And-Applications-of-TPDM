...
\begin{align*}
    ...
    &= \sum_{i<j}(\nu_{ij}^2 + c_{ij}).
\end{align*}
Since $\sum_{i<j}(\nu_{ij}^2+c_{ij})>0$ and $\|\hat{\bm{\sigma}}(n)\|_2^2>0$ almost surely, it follows that $\lambda^\star(n)>0$ for sufficiently large $n$. The above result gives that, asymptotically, the optimal shrinkage behaves as a constant divided by $k(n)\mathbb{E}[\|\hat{\bm{\sigma}}(n)\|_2^2]$. As the asymptotic constant is unknown, we eliminate it by taking take ratios at different values of $n$. Given a fixed proportion $\kappa \in (0,1)$, it is easy to see that
\begin{equation*}
