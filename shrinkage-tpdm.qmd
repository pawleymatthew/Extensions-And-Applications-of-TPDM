...
\end{align*}
This limit can be assumed to be strictly positive: assuming (as we are) that the bias of the sample TPDM is 'from above', then $C_{ij}>0$; $\sigma_{ij}$ is positive (a requirement for asymptotic normality of the empirical TPDM); the asympotic variances $\nu_{ij}^2$ are non-negative by definition. Moreover, noting that $\|\hat{\bm{\sigma}}(n)\|_2^2>0$ almost surely, it follows that $\lambda^\star(n)>0$ for sufficiently large $n$. The above result gives that, asymptotically, the optimal shrinkage behaves as a constant divided by $k(n)\mathbb{E}[\|\hat{\bm{\sigma}}(n)\|_2^2]$. As the asymptotic constant is unknown, we eliminate it by taking ratios at different values of $n$. Given a fixed proportion $\kappa \in (0,1)$, it is easy to see that
\begin{equation*}
...
For fixed $n$, selection of $\kappa$ is subject to the familiar bias-variance trade-off. If $\kappa$ is small, then $\left\lfloor \kappa n \right\rfloor$ may be too small for the underlying asymptotic approximation to be valid. If $\kappa$ is very close to one, then the estimate $\hat{\lambda}$ will be quite sensitive to small changes in $\hat{\zeta}$. This is borne out in our simulation experiments. A formal analysis of how to select $\kappa$ is beyond the scope of our investigation. We emphasise, however, that the choice of $\kappa$ is not simply replacing one unknown parameter ($\lambda$) with another. Roughly speaking, supposing $n$ is (sufficiently) large, one could simply take $\kappa=0.8$, say, and the tuning procedure will return good estimates; in contrast, the analogous approach of fixing $\lambda=0.05$, say, is unlikely to yield anything but poor results. 
